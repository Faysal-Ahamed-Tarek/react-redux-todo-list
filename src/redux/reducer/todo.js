...
export const TodoList_Reducer = (state = JSON.parse(localStorage.getItem('TodoList')) || TodoList,action) => {
    switch (action.type) {
        case "Input_Value": return {...state,InputController: {id:new Date().getTime(),TodoContent: action.payload}};
        case "Add_todo_item": return {...state,TodoItem:[...state.TodoItem,state.InputController],InputController : {id:"",TodoContent:""}};
        case "Delete_Todo": {
            const Updated_ToDo = state.TodoItem.filter((Item) => Item.id !== action.payload);
